...
Tensor TransformBody(const Tensor& tensor,
                     std::function<PrimExpr(const PrimExpr&, const Array<IterVar>&)> func) {
...
Tensor TransformBody(const Tensor& tensor, std::function<PrimExpr(const PrimExpr&)> func) {
  return TransformBody(tensor, [func](const PrimExpr& e, const Array<IterVar>&) { return func(e); });
}
...
Tensor InlineTailCall(const Tensor& tensor) {
  return TransformBody(tensor, InlineThisCall);
}
